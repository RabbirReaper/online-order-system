...
export const createAdmin = async (currentAdmin, adminData) => {
  ...
  if (adminData.role.startsWith('primary_')) {
    const existingPrimary = await checkPrimaryRoleConflict(
      adminData.role,
      processedData.brand,
      processedData.store,
    )
...
export const updateAdmin = async (adminId, currentAdmin, updateData) => {
  ...
  if (updateData.role && updateData.role !== admin.role) {
    ...
    if (updateData.role.startsWith('primary_')) {
      const existingPrimary = await checkPrimaryRoleConflict(
        updateData.role,
        updateData.brand || admin.brand,
        updateData.store || admin.store,
        adminId,
      )
      if (existingPrimary) {
        throw new AppError('此層級已存在主管理員', 400)
  ...
  const processedData = await processAdminData(
    currentAdmin,
    { ...admin.toObject(), ...updateData },
    admin,
  )
...
async function processAdminData(currentAdmin, adminData, existingAdmin = null) {
  ...
  if (['primary_system_admin', 'system_admin'].includes(targetRole)) {
    ...
    if (
      currentScope === 'brand' &&
      processedData.brand.toString() !== currentAdmin.brand.toString()
    ) {
    ...
    if (
      currentScope === 'store' &&
      processedData.store.toString() !== currentAdmin.store.toString()
    ) {
