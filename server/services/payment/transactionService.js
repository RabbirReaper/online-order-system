...
class TransactionService {
   ...
   */
  async linkOrder(transactionId, orderId) {
    try {
   ...
   * @returns {Promise<Object>} 更新後的交易
   */
  async markAsProcessing(transactionId, processorResponse = null) {
    try {
      const transaction = await this.findByTransactionId(transactionId)
   ...
   * @returns {Promise<Object>} 更新後的交易
   */
  async markAsCompleted(transactionId, processorResponse = null) {
    try {
      const transaction = await this.findByTransactionId(transactionId)
   ...
   * @returns {Promise<Object>} 更新後的交易
   */
  async markAsFailed(transactionId, reason, processorResponse = null) {
    try {
      const transaction = await this.findByTransactionId(transactionId)
   ...
   * @returns {Promise<Object>} 更新後的交易
   */
  async markAsCancelled(transactionId) {
    try {
      const transaction = await this.findByTransactionId(transactionId)
   ...
   */
  async getSuccessfulTransactionTotal(startDate, endDate) {
    try {
   ...
   */
  async getPendingOrderTransactions() {
    try {
