...
export const getAllVoucherTemplates = async (brandId) => {
  const templates = await VoucherTemplate.find({ brand: brandId }).sort({ createdAt: -1 })
...
export const getUserVouchers = async (userId, options = {}) => {
  ...
  const vouchers = await VoucherInstance.find(query)
    ...
    .populate('createdBy', 'name templateId') // populate BundleInstance 資訊
    .sort({ createdAt: -1 })
...
export const autoCreateVoucherTemplatesForDishes = async (brandId) => {
  try {
    ...
    const createdTemplatesInfo = newVoucherTemplates.map((template) => {
      const dish = dishesWithoutVouchers.find(
        (d) => d._id.toString() === template.exchangeDishTemplate.toString(),
      )
      return {
        id: template._id,
        name: template.name,
