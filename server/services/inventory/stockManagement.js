 ...
 * @returns {Promise<Object|null>} 庫存項目
 */
export const getInventoryItemByDishTemplate = async (storeId, dishTemplateId) => {
  const inventoryItem = await Inventory.findOne({
    store: storeId,
...
export const updateInventory = async (updateData, adminId) => {
  ...
  if (stock !== undefined) {
    ...
    if (
      (stockType === 'availableStock' || stockType === 'both') &&
      inventoryItem.enableAvailableStock
    ) {
      // 如果是單獨設定 availableStock，使用傳入的值
      // 如果是 both，則使用傳入的 stock 值
      const newAvailableStock =
        updateData.availableStock !== undefined ? updateData.availableStock : stock
...
export const reduceStock = async (reduceData) => {
  const { storeId, inventoryId, quantity, reason, orderId, adminId } = reduceData
...
export const reduceInventoryForOrder = async (order) => {
  try {
    ...
    for (const item of order.items) {
      // 獲取餐點實例以取得模板ID
      const DishInstance = (await import('../../models/Dish/DishInstance.js')).default
      const dishInstance = await DishInstance.findById(item.dishInstance)

      if (!dishInstance) {
        console.warn(`餐點實例 ${item.dishInstance} 不存在，跳過庫存扣除`)
        continue
      }

      // 查找對應的庫存項目
      const inventoryItem = await getInventoryItemByDishTemplate(
        order.store,
        dishInstance.templateId,
      )

      if (!inventoryItem) {
        console.warn(
          `餐點模板 ${dishInstance.templateId} 在店鋪 ${order.store} 沒有庫存記錄，跳過庫存扣除`,
        )
        continue
      }
...
export const addStock = async (addData) => {
  const { storeId, inventoryId, stockType = 'totalStock', quantity, reason, adminId } = addData
...
export const setAvailableStock = async (setData) => {
  const { storeId, inventoryId, availableStock, reason, adminId } = setData
...
export const restoreInventoryForCancelledOrder = async (order, adminId = null) => {
  ...
  for (const log of stockLogs) {
    ...
    inventoryItem.totalStock -= log.changeAmount
...
export const processDamage = async (damageData) => {
  const { storeId, inventoryId, stockType = 'totalStock', quantity, reason, adminId } = damageData
...
export const toggleSoldOut = async (storeId, inventoryId, isSoldOut, adminId) => {
  ...
  const test = await StockLog.create({
    ...
    itemName: inventoryItem.itemName,
    previousStock:
      inventoryItem[inventoryItem.enableAvailableStock ? 'availableStock' : 'totalStock'],
