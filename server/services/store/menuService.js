...
export const getMenuById = async (storeId, menuId, includeUnpublished = false) => {
  ...
  // 查詢特定菜單
  const menu = await Menu.findOne({ _id: menuId, store: storeId }).populate([
    {
      path: 'categories.items.dishTemplate',
      model: 'DishTemplate',
      select: 'name description basePrice image tags optionCategories',
    ...
    if (existingActiveMenu) {
      const typeText =
        menuData.menuType === 'food'
          ? '現金購買餐點'
          : menuData.menuType === 'cash_coupon'
            ? '現金購買預購券'
            : menuData.menuType === 'point_exchange'
              ? '點數兌換'
              : menuData.menuType
      throw new AppError(
        `此店鋪已有啟用的「${typeText}」類型菜單：${existingActiveMenu.name}。請先停用現有菜單或將新菜單設為停用狀態。`,
        400,
      )
    }
  }
...
export const updateMenu = async (storeId, menuId, updateData) => {
  ...
  if (updateData.menuType && updateData.menuType !== currentMenu.menuType && currentMenu.isActive) {
    ...
    if (existingActiveMenu) {
      const typeText =
        updateData.menuType === 'food'
          ? '現金購買餐點'
          : updateData.menuType === 'cash_coupon'
            ? '現金購買預購券'
            : updateData.menuType === 'point_exchange'
              ? '點數兌換'
              : updateData.menuType
      throw new AppError(
        `此店鋪已有啟用的「${typeText}」類型菜單：${existingActiveMenu.name}。請先停用該菜單再進行類型變更。`,
        400,
      )
    }
  }
  ...
  // 查找並更新菜單
  const menu = await Menu.findOneAndUpdate({ _id: menuId, store: storeId }, updateData, {
    new: true,
    runValidators: true,
  })
 ...
 * @returns {Promise<Object>} 菜單物件
 */
export const getMenuAllPopulateById = async (storeId, menuId, includeUnpublished = false) => {
  // 檢查店鋪是否存在
  const store = await Store.findById(storeId)
