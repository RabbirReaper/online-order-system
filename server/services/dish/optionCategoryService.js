 ...
 * 獲取類別使用情況
 * @param {String} categoryId - 類別ID
 * @returns {Promise<Object>} 使用情況
 */
export const getCategoryUsage = async (categoryId) => {
  // 檢查類別是否存在
  const category = await OptionCategory.findById(categoryId);
  ...
  const templates = await DishTemplate.find({
    'optionCategories.categoryId': categoryId
  }).select('name');
...
/**
 * 批量創建選項類別
 * @param {Array} categoriesData - 類別數據陣列
 * @returns {Promise<Array>} 創建的選項類別列表
 */
export const bulkCreateCategories = async (categoriesData) => {
  if (!Array.isArray(categoriesData) || categoriesData.length === 0) {
    throw new AppError('無效的類別數據', 400);
  }
  ...
  for (const categoryData of categoriesData) {
    ...
    const existingCategory = await OptionCategory.findOne({ name: categoryData.name });
    if (existingCategory) {
      throw new AppError(`類別名稱 '${categoryData.name}' 已存在`, 400);
    }
