...
const testDateRanges = () => {
  ...
  try {
    ...
    periods.forEach(period => {
      ...
      console.log(`${period}: ${range.start.toFormat('yyyy-MM-dd HH:mm')} ~ ${range.end.toFormat('yyyy-MM-dd HH:mm')}`);
    });
...
const testValidation = () => {
  ...
  try {
    ...
    // 測試有效時區
    console.log(`有效時區 "Asia/Taipei": ${dateUtils.validateTimezone('Asia/Taipei') ? '✅' : '❌'}`);
    console.log(`無效時區 "Invalid/Zone": ${dateUtils.validateTimezone('Invalid/Zone') ? '❌' : '✅'}`);
