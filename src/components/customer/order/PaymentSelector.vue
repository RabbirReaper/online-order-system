      ...
      <div class="card-body">
        ...
        <div class="payment-type-selector mb-4">
          ...
          <BButtonGroup class="w-100">
            <BButton v-for="type in paymentTypes" :key="type.value"
              :variant="paymentType === type.value ? 'primary' : 'outline-secondary'" class="payment-type-btn w-50"
        ...
        <div class="payment-method-options">
          ...
          <BFormRadioGroup v-model="localModelValue" :options="availablePaymentMethods" stacked name="payment-methods"
            buttons button-variant="outline-primary" class="w-100"></BFormRadioGroup>
        </div>

        <!-- 現金支付說明 -->
        <div v-if="localModelValue === 'cash'" class="payment-explanation mt-3 p-3 bg-light rounded">
          <div class="d-flex align-items-start">
            <i class="bi bi-cash me-2 mt-1 text-success"></i>
            <div>
              <div class="fw-bold mb-1">現金付款</div>
              <p class="mb-0 small text-muted">
                請於取餐時支付。我們接受新台幣現金支付，可找零。
              </p>
            </div>
          </div>
        </div>
        ...
        <!-- 信用卡支付說明 -->
        <div v-else-if="localModelValue === 'credit_card'" class="payment-explanation mt-3 p-3 bg-light rounded">
          <div class="d-flex align-items-start">
            <i class="bi bi-credit-card me-2 mt-1 text-primary"></i>
            <div>
              <div class="fw-bold mb-1">信用卡付款</div>
              <p class="mb-0 small text-muted">
                {{ paymentType === 'On-site' ? '請於取餐時刷卡。我們接受 VISA、MasterCard、JCB 等主要信用卡。' : '將轉跳至信用卡支付頁面，完成線上付款。' }}
              </p>
            </div>
          </div>
        </div>
        ...
        <!-- LINE Pay 支付說明 -->
        <div v-else-if="localModelValue === 'line_pay'" class="payment-explanation mt-3 p-3 bg-light rounded">
          <div class="d-flex align-items-start">
            <i class="bi bi-phone me-2 mt-1 text-success"></i>
            <div>
              <div class="fw-bold mb-1">LINE Pay 支付</div>
              <p class="mb-0 small text-muted">
                {{ paymentType === 'On-site' ? '請於取餐時出示 LINE Pay QR Code 進行掃描付款。' : '將轉跳至 LINE Pay 頁面，完成線上付款。' }}
              </p>
            </div>
          </div>
        </div>
        ...
        <!-- 其他支付方式說明 -->
        <div v-else-if="localModelValue === 'other'" class="payment-explanation mt-3 p-3 bg-light rounded">
          <div class="d-flex align-items-start">
            <i class="bi bi-wallet2 me-2 mt-1 text-secondary"></i>
            <div>
              <div class="fw-bold mb-1">其他付款方式</div>
              <p class="mb-0 small text-muted">
                {{ paymentType === 'On-site' ? '請於取餐時詢問店員可用的其他支付方式。' : '將轉跳至第三方支付頁面，完成線上付款。' }}
              </p>
            </div>
          </div>
        </div>
...
// 監聽支付類型變化，確保支付方式合法
watch(() => props.paymentType, (newType) => {
