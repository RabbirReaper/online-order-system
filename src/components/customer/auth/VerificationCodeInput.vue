<template>
  <div class="verification-code-container">
    ...
    <div class="code-input-group">
      <input v-for="(digit, index) in codeArray" :key="index" type="text" class="code-input" maxlength="1"
        :value="digit" @input="updateDigit($event, index)" @keydown="handleKeydown($event, index)"
        @focus="$event.target.select()" :ref="el => { if (el) digitRefs[index] = el }" />
    </div>
