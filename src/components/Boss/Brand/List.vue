        ...
        <BCard class="h-100">
          <div class="position-relative overflow-hidden" style="height: 200px">
            ...
            <div class="position-absolute top-0 end-0 p-2">
              <BBadge variant="success" v-if="brand.storesCount > 0"
                >{{ brand.storesCount }} 間店鋪</BBadge
              >
            </div>
          </div>
          <BCardBody>
            ...
            <BCardText v-if="brand.description">{{
          ...
          <BCardFooter class="bg-transparent border-top-0">
            <div class="d-flex flex-wrap">
              <BButtonGroup class="mb-2 me-2">
                <router-link
                  :to="{ name: 'brand-detail', params: { id: brand._id } }"
                  class="btn btn-outline-primary btn-sm"
                >
                  <i class="bi bi-eye me-1"></i>查看
                </router-link>
                <router-link
                  :to="{ name: 'brand-admin-dashboard', params: { brandId: brand._id } }"
                  class="btn btn-outline-primary btn-sm"
                >
