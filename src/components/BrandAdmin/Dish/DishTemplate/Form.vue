<template>
  <div class="card">
        ...
        <div class="mb-4">
          <h6 class="border-bottom pb-2 mb-3 d-flex justify-content-between">
            ...
            <button
              ...
              @click="showAddOptionCategoryModal"
            >
    ...
    <div
      class="modal fade"
      id="addOptionCategoryModal"
      tabindex="-1"
      aria-labelledby="addOptionCategoryModalLabel"
      aria-hidden="true"
    >
          ...
          <div class="modal-body">
            <div class="mb-3">
              ...
              <div class="form-text" v-if="availableCategories.length === 0">
                沒有可用的選項類別，請先在
                <router-link :to="`/admin/${brandId}/option-categories/create`"
                  >選項類別管理</router-link
                >
                頁面中創建
              </div>
...
const updateAvailableCategories = () => {
  ...
  availableCategories.value = allCategories.filter(
    (category) => !usedCategoryIds.includes(category._id),
  )
}
...
const submitForm = async () => {
  ...
  try {
    ...
    if (isEditMode.value) {
      // 更新餐點
      response = await api.dish.updateDishTemplate({
        id: route.params.id,
        data: submitData,
        brandId: brandId.value,
      })
