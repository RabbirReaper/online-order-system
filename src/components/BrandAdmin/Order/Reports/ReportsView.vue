      ...
      <div class="card-body">
        <div class="row align-items-center">
          ...
          <div class="col-md-4">
            ...
            <select class="form-select" v-model="filters.selectedStoreId" @change="handleFilterChange">
...
const customerRetentionData = computed(() => {
  ...
  return {
    labels: days.map(day => day.toLocaleDateString('zh-TW', { month: '2-digit', day: '2-digit' })),
...
const fetchStores = async () => {
  try {
    ...
    if (response && response.stores) {
      ...
      console.log('店鋪列表:', stores.value.map(s => ({ id: s._id, name: s.name })));
    } else {
