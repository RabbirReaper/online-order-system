<template>
  <div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
      <h4 class="mb-0">{{ isEditMode ? '編輯兌換券' : '新增兌換券' }}</h4>
    ...
    <div class="card">
      <div class="card-body">
        <form @submit.prevent="submitForm">
          ...
          <div class="mb-4">
            ...
            <div class="row">
              ...
              <div class="col-md-6 mb-3">
                ...
                <input type="text" class="form-control" id="sellingPoint" v-model="formData.sellingPoint"
                  maxlength="200" placeholder="請輸入兌換券賣點（選填）">
                <div class="form-text">簡短描述此兌換券的特色，最多200個字元</div>
              </div>
            </div>
            ...
            <div class="mb-3">
              ...
              <textarea class="form-control" id="description" v-model="formData.description" rows="3"
                placeholder="請輸入兌換券詳細描述（選填）"></textarea>
              <div class="form-text">向顧客詳細說明此兌換券的內容和使用方式</div>
            </div>
            ...
            <div class="row">
              <div class="col-md-6 mb-3">
                ...
                <div class="form-text">停用後顧客無法購買此兌換券</div>
              </div>
              ...
              <div class="col-md-6 mb-3">
                ...
                <div class="form-text">根據有效期自動啟用/停用兌換券</div>
              </div>
            </div>
          </div>
          ...
          <!-- 圖片上傳 -->
          <div class="mb-4">
            <h6 class="border-bottom pb-2 mb-3">兌換券圖片</h6>
            <div class="row">
              <div class="col-md-6">
          ...
          <div class="mb-4">
            ...
            <div class="row" v-if="formData.autoStatusControl">
              <div class="col-md-6 mb-3">
                <label for="validFrom" class="form-label">有效期起始</label>
                <input type="datetime-local" class="form-control" id="validFrom" v-model="formData.validFrom">
                <div class="form-text">兌換券開始販售的時間</div>
              </div>

              <div class="col-md-6 mb-3">
                <label for="validTo" class="form-label">有效期結束</label>
                <input type="datetime-local" class="form-control" id="validTo" v-model="formData.validTo">
                <div class="form-text">兌換券停止販售的時間</div>
              </div>
            </div>
          ...
          <div class="mb-4">
            ...
            <!-- 兌換項目列表 -->
            <div v-if="formData.bundleItems.length > 0">
              <div v-for="(item, index) in formData.bundleItems" :key="index" class="card mb-3">
                <div class="card-body">
                  <div class="row">
                    <div class="col-md-8 mb-3">
                      <label class="form-label required">兌換券模板</label>
                      <select v-model="item.couponTemplate" class="form-select"
                        :class="{ 'is-invalid': getItemError(index, 'couponTemplate') }">
                        <option value="">選擇兌換券模板</option>
                        <option v-for="template in couponTemplates" :key="template._id" :value="template._id">
                          {{ template.name }} - {{ template.pointCost }} 點
                        </option>
                      </select>
                      <div class="invalid-feedback" v-if="getItemError(index, 'couponTemplate')">
                        {{ getItemError(index, 'couponTemplate') }}
                      </div>
                    </div>
          ...
          <div class="d-flex justify-content-end">
            ...
            <button type="submit" class="btn btn-primary" :disabled="isSubmitting">
              ...
              {{ isSubmitting ? '處理中...' : (isEditMode ? '更新兌換券' : '建立兌換券') }}
            </button>
          </div>
        </form>
      </div>
...
const validateForm = () => {
  ...
  if (hasCashPrice.value && (!formData.cashPrice.original || formData.cashPrice.original <= 0)) {
    formErrors.value.push('現金原價必須大於 0');
    isValid = false;
  }

  if (hasPointPrice.value && (!formData.pointPrice.original || formData.pointPrice.original <= 0)) {
  ...
  if (formData.bundleItems.length === 0) {
    formErrors.value.push('請至少添加一個兌換項目');
    isValid = false;
  } else {
    formData.bundleItems.forEach((item, index) => {
      if (!item.couponTemplate) {
        errors[`bundleItems.${index}.couponTemplate`] = '請選擇兌換券模板';
  ...
  if (formData.autoStatusControl) {
    if (!formData.validFrom || !formData.validTo) {
      formErrors.value.push('啟用自動狀態控制時，需要設定有效期間');
      isValid = false;
    } else if (new Date(formData.validFrom) >= new Date(formData.validTo)) {
      formErrors.value.push('有效期起始時間必須早於結束時間');
      isValid = false;
    }
  }
...
const fetchCouponTemplates = async () => {
  ...
  try {
    ...
    if (response && response.templates) {
      // 只取兌換券類型的模板
      couponTemplates.value = response.templates.filter(template =>
...
const fetchVoucherData = async () => {
  ...
  try {
    ...
    if (response && response.bundle) {
      ...
      formErrors.value = ['獲取兌換券資料失敗'];
      setTimeout(() => {
        router.push(`/admin/${brandId.value}/vouchers`);
      }, 2000);
    }
  } catch (error) {
    console.error('獲取兌換券資料時發生錯誤:', error);
    formErrors.value = ['獲取兌換券資料時發生錯誤，請稍後再試'];
    setTimeout(() => {
      router.push(`/admin/${brandId.value}/vouchers`);
    }, 2000);
  }
...
const submitForm = async () => {
  ...
  try {
    ...
    console.error('儲存兌換券時發生錯誤:', error);
    ...
    if (error.response && error.response.data) {
      ...
      formErrors.value = ['儲存兌換券時發生未知錯誤，請稍後再試'];
    }
