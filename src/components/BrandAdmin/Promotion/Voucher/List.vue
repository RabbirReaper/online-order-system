            ...
            <tbody>
              <tr v-for="template in filteredTemplates" :key="template._id">
                ...
                <td>
                  <div class="btn-group">
                    <router-link
                      :to="`/admin/${brandId}/vouchers/detail/${template._id}`"
                      class="btn btn-sm btn-outline-primary"
                    >
                      <i class="bi bi-eye me-1"></i>查看
                    </router-link>
                    <router-link
                      :to="`/admin/${brandId}/vouchers/edit/${template._id}`"
                      class="btn btn-sm btn-outline-primary"
                    >
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <p v-if="templateToDelete">您確定要刪除兌換券模板「{{ templateToDelete.name }}」嗎？</p>
          </div>
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">取消</button>
            <button
    ...
    filtered = filtered.filter(
      (template) =>
        ...
        (template.exchangeDishTemplate?.name &&
          template.exchangeDishTemplate.name.toLowerCase().includes(query)),
    )
  }
